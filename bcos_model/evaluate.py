...
def get_parser(add_help=True):
    ...
    parser.add_argument(
        "--dataset",
        choices=["ImageNet", "CIFAR10",'celebA','waterbirds'],
        default="ImageNet",
        help="The dataset.",
    )
    print("added the arg")
...
def evaluate(model, data_loader):
    ...
    samples_per_class = torch.zeros(num_classes, dtype=torch.long)
    ...
    # per class accuracy
    accuracy_per_class = correct_per_class.float() / samples_per_class.float()
    for i, acc in enumerate(accuracy_per_class):
        print(f"Class {data_loader.dataset.classes[i]}: {acc:.3%}")
